...
| `ARCHON_REPLAY_MAX_MESSAGES` | Messages replayed into the prompt when a conversation's assistant session is lost (a restart) and the turn has to be answered from Archon's own history. Newest first. | `30` |
| `ARCHON_REPLAY_MAX_CHARS` | Total characters across that replayed history. Roughly 3 000 tokens at the default, paid once per conversation per restart. | `12000` |
| `ARCHON_REPLAY_MAX_MESSAGE_CHARS` | Longest a single replayed message may be before it is cut short and marked `… [truncated]`. | `1200` |
